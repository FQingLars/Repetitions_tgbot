...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(message)s', handlers=[logging.FileHandler(f"Logs/app{int(time.time())}.log", encoding='utf-8'),logging.StreamHandler()])
...
def start(message):
    ...
    kb.add(bttns[0], bttns[1]).add(bttns[2]).add(bttns[3], bttns[4])
...
def request_rep_add(message: Message, user_id: int, chat_id: int):
    ...
    if check_admin(user_id):
        ...
        for admin_id in admins:
            bot.send_message(admin_id, f"Поступил запрос на репетицию:\n\n{text}", reply_markup=kb)
...
if __name__ == "__main__":
    if not os.path.isfile("repdatabase.db"):
        primary_admin = int(input("Введите chat id первичного админа для добавления его в админы. Узнать его можно, переслав его сообщение этому боту: @GetChatID_IL_BOT.\n"))
        db_init(primary_admin)

    autocleaner()
    logging.info("Бот запущен в непрерывном поллинге.")
